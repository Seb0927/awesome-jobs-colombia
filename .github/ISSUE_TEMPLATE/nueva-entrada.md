---
name: Nueva entrada
about: Agregar una nueva empresa a la lista
title: "Agregar <nombre empresa>"
labels: ["adición"]
assignees: ""
---

## Confirmación

- [ ] Confirmo que esta empresa tiene procesos activos de contratación en Colombia

## Donde ofrece sus servicios

<!-- Selecciona donde ofrece servicios la empresa, pueden ser múltiples opciones -->
<!-- *`(LATAM)`*: La empresa ofrece posiciones remotas para desarrolladores en lationamérica -->
<!-- *`(Colombia)`*: La empresa ofrece posiciones remotas y/o posiciones en mas de cinco ciudades de Colombia -->
<!-- *`(<Ciudad>)`*: La empresa ofrece oportunidades laborales para una ciudad determinada -->

- [ ] LATAM
- [ ] Colombia
- [ ] Ciudad(es) específica(s): <!-- Listar ciudades -->

## Enlace a página de trabajo

<!-- Coloca el enlace directo a la página de carreras/empleos de la empresa -->

## Información adicional

<!-- Cualquier información adicional relevante sobre la empresa -->